...
def check_low_stock():
    ...
    for item in low_items:
        existing = Alarm.query.filter_by(alarm_type='low_stock', stock_item_id=item.id, is_read=False).first()
        if not existing:
            alarm = Alarm(
...
def inject_globals():
    try:
        ...
        return {'all_service_types': [], 'unread_alarms_count': 0}
...
def dashboard():
    if current_user.role == 'admin':
        ...
        return render_template('admin_panel.html', empleados=empleados, informes=informes, 
                             inventory=inventory, clients=clients, services=services, alarms=alarms)
    ...
    return render_template('tech_panel.html', today_date=date.today().strftime('%Y-%m-%d'), 
                           stock_items=stock_items, pending_tasks=pending_tasks)
...
def manage_services():
    ...
    action = request.form.get('action')
    if action == 'add':
        if not ServiceType.query.filter_by(name=request.form['name']).first():
...
def schedule_appointment():
    ...
    try:
        ...
        notes = request.form.get('notes', '')
        new_task = Task(
            tech_id=tech_id, client_name=client_name, date=task_date,
            start_time=time, service_type=service_type, description=notes, status='Pendiente'
        )
...
def create_appointment():
    try:
        ...
        notes = request.form.get('notes', '')
        new_task = Task(
            tech_id=current_user.id, client_name=client_name, date=task_date,
            start_time=time, service_type=service_type, description=notes, status='Pendiente'
        )
        db.session.add(new_task)
        db.session.commit()
...
def save_report():
    try:
        ...
        if stock_item_id and stock_item_id != '':
            ...
            if stock_item:
                if stock_action in ['used', 'removed']:
        ...
        if linked_task_id and linked_task_id != 'none':
            ...
            if task:
                ...
                if attachments_list:
                    ...
                    if task.attachments:
                        try:
                            existing = json.loads(task.attachments)
                        except:
                            pass
                    existing.extend(attachments_list)
            ...
            new_task = Task(
                tech_id=current_user.id, client_name=client_name, date=task_date,
                start_time=entry_time, end_time=exit_time, service_type=service_type,
                description=description, parts_text=parts_text, stock_item_id=stock_item_id,
                stock_quantity_used=stock_quantity, stock_action=stock_action, status='Completado',
                signature_data=signature_data, attachments=json.dumps(attachments_list) if attachments_list else None,
                actual_start_time=datetime.now(), actual_end_time=datetime.now()
            )
            db.session.add(new_task)
            db.session.commit()
            check_low_stock()
...
def start_task(task_id):
    ...
    db.session.commit()
    return jsonify({'success': True, 'msg': 'Trabajo iniciado', 'start_time': task.start_time})
...
def get_clients():
    ...
    return jsonify([{
        ...
        'address': c.address, 'has_support': c.has_support
    } for c in clients])
...
def search_clients():
    ...
    return jsonify([{'id': c.id, 'name': c.name, 'phone': c.phone, 'email': c.email, 'address': c.address} for c in clients])
...
def get_task(task_id):
    ...
    return jsonify({
        'success': True,
        'data': {
            'client_name': task.client_name, 'date': task.date.strftime('%Y-%m-%d'),
            'time': task.start_time, 'service_type': task.service_type, 'notes': task.description
        }
    })
...
def print_report(task_id):
    ...
    <body onload="window.print()">
        ...
        <div class="row">
            <div class="col-6"><div class="label">CLIENTE</div><div class="value">{task.client_name}</div></div>
            <div class="col-6"><div class="label">TÉCNICO</div><div class="value">{task.tech.username.upper() if task.tech else 'SIN TÉCNICO'}</div></div>
        </div>
        <div class="row mt-3">
            <div class="col-4"><div class="label">SERVICIO</div><div class="value">{task.service_type}</div></div>
            <div class="col-4"><div class="label">HORA ENTRADA</div><div class="value">{task.start_time}</div></div>
            <div class="col-4"><div class="label">HORA SALIDA</div><div class="value">{task.end_time}</div></div>
        </div>
        <div class="mt-4"><div class="label">DESCRIPCIÓN DEL TRABAJO</div><div class="box">{task.description}</div></div>
        <div class="row mt-4">
            <div class="col-12">
...
def format_events(tasks):
    ...
    for t in tasks:
        ...
        events.append({
            ...
            'extendedProps': {
                'status': t.status, 'client': t.client_name, 'desc': t.description,
                'tech_name': t.tech.username.upper() if t.tech else 'SIN TÉCNICO',
                'service_type': t.service_type, 'has_attachments': len(attachments_list) > 0
            }
        })
...
def get_task_details(task_id):
    ...
    return jsonify({
        'success': True,
        'data': {
            'id': task.id, 'client_name': task.client_name, 'date': task.date.strftime('%Y-%m-%d'),
            'start_time': task.start_time, 'end_time': task.end_time, 'service_type': task.service_type,
            'description': task.description, 'parts_text': task.parts_text, 'status': task.status,
            'tech_name': task.tech.username if task.tech else 'SIN TÉCNICO', 'attachments': attachments_list,
            'has_signature': bool(task.signature_data),
            'stock_info': {
                'item_name': task.stock_item.name if task.stock_item else None,
                'quantity': task.stock_quantity_used, 'action': task.stock_action
...
def get_tech_stats(tech_id):
    if current_user.role != 'admin' and current_user.id != tech_id:
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    ...
    for task in tasks:
        ...
        if service_type not in service_stats:
            service_stats[service_type] = {'count': 0, 'tasks': []}
        service_stats[service_type]['count'] += 1
        service_stats[service_type]['tasks'].append({
            'id': task.id, 'client': task.client_name, 'date': task.date.strftime('%d/%m/%Y'),
            'time': f"{task.start_time} - {task.end_time}", 'has_attachments': bool(task.attachments)
        })
    ...
    return jsonify({
        'success': True,
        'data': {
            'tech_name': tech.username, 'total_completed': len(tasks),
            'total_hours': round(total_time, 2), 'service_breakdown': service_stats
        }
    })
...
def get_alarms():
    ...
    alarms = Alarm.query.order_by(Alarm.is_read.asc(), Alarm.created_at.desc()).all()
    return jsonify([{
        'id': a.id, 'type': a.alarm_type, 'title': a.title, 'description': a.description,
        'client_name': a.client_name, 'created_at': a.created_at.strftime('%d/%m/%Y %H:%M'),
        'is_read': a.is_read, 'priority': a.priority
    } for a in alarms])
...
with app.app_context():
    ...
    if ServiceType.query.count() == 0:
        for s in [{'name': 'Avería', 'color': '#fd7e14'}, {'name': 'Revisión', 'color': '#0d6efd'},
                  {'name': 'Instalación', 'color': '#6f42c1'}, {'name': 'Otros servicios', 'color': '#20c997'}]:
    ...
    if Client.query.count() == 0:
        db.session.add(Client(
            name='Cliente Ejemplo', phone='900123456', email='ejemplo@cliente.com',
            address='Calle Ejemplo 1, Madrid', has_support=True, support_monday_friday=True
        ))
    db.session.commit()
